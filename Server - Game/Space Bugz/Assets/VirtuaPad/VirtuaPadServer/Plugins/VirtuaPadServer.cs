...
public class VirtuaPadServer<VPClientType> where VPClientType : VPClientBaseClass, new()
{
	...
	private Dictionary<byte, TcpEventSubscriber> tcpEventSubscribers = new Dictionary<byte, TcpEventSubscriber>();
	...
	public VirtuaPadServer( int tcpPort, int udpPort)
	{
		...
		{
			//TCP setup
			localAddr = IPAddress.Parse("192.168.225.2");
			tcpListener = new TcpListener(localAddr, tcpPort);
			tcpListener.Start();
		}
		catch
	...
	public void AddTcpEventSubscriber(byte tag, TcpEventSubscriber subscriber)
	{
	...
	public void UdpListen()
	{
		...
		while( udpOn )
		{
			...
			{
				//Debug.Log( e.ToString()  +  " and lots of cheese");
				//udpClient.Close();
				udpOn = false;
				break;
			}
			...
			{
				Debug.Log("aborted udp from inside thread");
				udpOn = false;
				break;
			}
			catch
			...
			//sleep so other processes has time to run
			//Thread.Sleep(30); // ms
		}
		
		
		Debug.Log("UDP thread ended");
	}
	...
	{
		Debug.Log("remove idle client thread started");
		while(removeClientsOn)
		{
			...
			//foreach(byte id in removedClients)
			//	Debug.Log("removed client " + id);
		}
		Debug.Log("remove idle client thread stopped");
	}
